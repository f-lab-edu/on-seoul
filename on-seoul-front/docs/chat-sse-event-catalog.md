...
> 의존하지 말고 **payload 키로 식별**한다(§4): `type:"title"`이면 title, `answer`가 있으면 final,
> 그 외는 진행으로 흡수.
...
> **payload 키/`type`**으로 식별한다(§4): `type==="title"`이면 **title**, `answer` 있고 `error` 없으면
...
| `title` | string | 생성된 대화 제목(비어 있지 않음 — 빈 값이면 이벤트 자체가 emit되지 않음). |
...
- **fail-open**: LLM/캐시 오류 시 이벤트를 생략하고 스트림은 정상 진행한다(제목 누락 ≠ 스트림 중단).
  이때 방 생성 시 백엔드가 넣은 폴백 제목(질문 앞 50자)을 그대로 유지한다.
...
2. 이벤트 수신 → payload JSON 파싱(SSE name 비의존):
   - `type === "title"` → **title**: 방 제목 표시(라이브 헤더) + 목록/이력 캐시 갱신(있으면). final과 무관하게 독립 처리, 종료 아님(순서 무관·미수신 가능).
