...
event:title            ← AI 서비스 대화 제목 (첫 턴, 0~1회)
...
> `event:decision`, `event:title`, `event:sources_update`, `event:final`) relay된다. 프론트는 이름에
> 의존하지 말고 **payload 키로 식별**한다(§4): `answer`가 있으면 final, `type:"title"`이면 title,
> 그 외는 진행으로 흡수.
...
| title | `title` | **AI 서비스** | 첫 턴 0~1회 | JSON `{ "type": "title", "room_id", "title", "message_id", "query" }` |
| sources_update | `sources_update` | **AI 서비스** | 0회 이상 | JSON `{ "event": "sources_update", "sources": [...] }` |
| final | `final` | **AI 서비스** | 정상 종료 1회 | AI 서비스 final payload JSON (`answer` + `service_cards` 등, **title 미포함**) |
| error | `error` | **API 서비스** | 오류 시 1회 | 사용자용 에러 메시지 **문자열**(JSON 아님) |

> **식별 규칙(이름 비의존)**: 이벤트는 이름을 유지한 채 relay되지만, 프론트는 SSE name이 아니라
> **payload 키**로 식별한다(§4): `answer` 있고 `error` 없으면 **final**, `type:"title"`이면 **title**,
> 그 외(progress·decision·sources_update·미지 이벤트)는 전부 **진행으로 흡수**한다. 이렇게 하면
> 미문서화/신규 이벤트도 코드 변경 없이 안전하다(forward-compatible). `init`/`error`만 이름으로 구분한다.
...
```jsonc
// final 예시 (title 미포함 — 제목은 별도 event:title 로 분리됨)
...
대화방의 **첫 턴(message_id == 1)** 에만 0~1회 emit된다. 제목 생성은 답변 파이프라인과
독립된 병렬 노드(`generate_title_node`)가 담당하므로, final보다 먼저 도착할 수도, 답변
파이프라인이 실패해도 독립적으로 도착할 수도 있다. final payload에는 더 이상 `title`이 없다.

```
event:title
data:{"type":"title","room_id":42,"title":"수영장 안내","message_id":1,"query":"수영장 알려줘"}
```

| 필드 | 타입 | 의미 |
|---|---|---|
| `type` | string | 항상 `"title"`. **식별자**(SSE name relay 유실 대비 — 이 키로 title 이벤트를 판정). |
| `room_id` | number(long) | 제목이 귀속되는 방 ID. |
| `title` | string | 생성된 대화 제목(10자 이내, 비어 있지 않음 — 빈 값이면 이벤트 자체가 emit되지 않음). |
| `message_id` | number | 제목을 만든 턴의 메시지 ID(첫 턴이므로 항상 1). |
| `query` | string | 제목 생성에 쓰인 사용자 원본 질문. |

- **식별**: payload `type == "title"` 로 판정한다(SSE name 비의존).
- **첫 턴 전용**: 후속 턴(message_id != 1)에는 emit되지 않는다.
- **fail-open**: LLM/캐시 오류 시 이벤트를 생략하고 스트림은 정상 진행한다(제목 누락 ≠ 스트림 중단).
- 정본: `on-seoul-agent/agents/nodes/title.py`(`generate_title_node`), `on-seoul-agent/routers/chat.py`.
...
2. 이벤트 수신 → payload JSON 파싱(SSE name 비의존):
   ...
   - `type == "title"` → **title**: `title`(신규 방 첫 메시지)로 목록 캐시/방 제목 갱신. final과 무관하게 독립 처리.
