...
Vercel은 이 모든 역할을 플랫폼 수준에서 자동으로 처리한다.  
전 세계 여러 리전에 분산된 글로벌 CDN을 통해 동일한 기능을 제공하므로, 별도의 Nginx 설정·유지보수가 불필요하다.
...
> Vercel 커스텀 도메인은 Cloudflare Proxy를 비활성화(DNS only)해야 한다.  
> Proxy를 활성화하면 Vercel의 TLS 인증서 자동 발급 과정과 충돌할 수 있다.
