...
async def sql_search(
    ...
    *,
    ...
    max_class_name:
        대분류 카테고리 리스트 필터 (예: ["체육시설"] 또는 제외 표현의 여집합
        ["문화체험","공간시설","교육강좌","진료복지"]). max_class_name = ANY(:classes)
        로 다중 카테고리 OR 매칭. None/빈 리스트면 미적용.
    area_name:
    ...
    # max_class_name 다중값: max_class_name = ANY(:classes) 로 여러 카테고리를 OR
    # 매칭한다("체육시설 말고"의 여집합 4종 포함). 값은 리스트째 bind(인젝션 방지).
    # 방어: 스칼라 str 이 새어들어와도 chars 로 쪼개지지 않게 감싼다(area_name 동일 패턴).
