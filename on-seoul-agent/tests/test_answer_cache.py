...
class TestEmptyStateTTL:
    ...
    async def test_vector_empty_sql_present_uses_normal_ttl(self, mock_redis, sample_payload):
        """한쪽만 empty 면 정상 TTL."""
        asym_state = {"refined_query": "x", "vector_results": [], "sql_results": [{"a": 1}]}
        from core.cache import set_cached_answer
        from core.config import settings
    ...
    async def test_sql_empty_vector_present_uses_normal_ttl(self, mock_redis, sample_payload):
        asym_state = {"refined_query": "x", "vector_results": [{"service_id": "S1"}], "sql_results": []}
        from core.cache import set_cached_answer
        from core.config import settings
        await set_cached_answer("x", sample_payload, asym_state, mock_redis)
        assert mock_redis.set.call_args.kwargs["ex"] == settings.answer_cache_ttl
