...
버킷 정의는 계획서 §6 Phase 0 를 따른다:
  - 규칙 자동 라벨(공짜, state 신호): ZERO_HIT / THIN / SKEW / RETRIED / NORMAL
...
class QuerySignals(BaseModel):
    ...
    action: str | None = Field(default=None, description="triage action")
...
class BucketDistribution(BaseModel):
    ...
    l1_demand / l2_demand 는 계획서 결정 게이트 정의에 따라 파생한다.
    ...
    l1_demand: int = Field(description="단일-intent 실패 합계(L1 수요)")
    l2_demand: int = Field(description="복합-표현불가 합계(L2 수요)")
