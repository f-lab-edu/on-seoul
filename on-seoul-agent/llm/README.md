...
Gemini `gemini-embedding-2-preview` 모델을 사용하며 `output_dimensionality=768`으로 고정합니다. DDL의 `vector(768)` 컬럼과 일치합니다.
...
emb = Embedder()
vector = await emb.embed("서울 수영장")                        # list[float] (len=768)
