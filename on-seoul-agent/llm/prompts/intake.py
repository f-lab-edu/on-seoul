...
도메인 무관 (oos_type=domain_outside):
  ...
  단, 예약 시설의 *운영여부*를 기상 조건과 함께 묻는 것("폭염에도 운영해?")은
  운영성 질문이라 도메인 안이다. 순수 날씨 정보 요청("오늘 날씨 어때?")만 domain_outside 다.
...
맥락 암시형 후속 (핵심 — NEW 오분류 방지):
  (1) 정정/부정 후속 → REFINE. "아니 X 말이야", "그게 아니라 X", "X 말고", "내 말은 X"
      처럼 직전 흐름을 *바로잡아 topic 을 다시 선언*하는 후속은 REFINE 이다(NEW 아님).
      어시스턴트가 엉뚱한 방향으로 표류한 뒤 사용자가 원 의도를 다시 못박는 경우로,
      새 topic 을 도입하는 게 아니라 직전 제약 위에 올바른 topic 을 재선언하는 것이다.
      예: 어시스턴트가 "성동구 서비스"로 표류 → 사용자 "아니 영화촬영 장소 말이야"
          → REFINE(직전 지역 제약 유지 + topic=영화촬영 재선언). NEW 로 보지 말 것.
  (2) 생략형 속성 후속 → DRILL. "주말도 돼?", "유료야?", "몇 시까지?" 처럼 시설명·
      서수 없이 *속성만* 묻고 새 시설/지역/카테고리를 도입하지 않으면, 직전 초점 항목
      (prev_entities)에 바인딩하는 DRILL 이다.
      · 직전이 단일 항목(단일 describe/DRILL)이면 그 항목 인덱스에 바인딩. ref_indices=[1].
      · prev_entities 가 다중인데 어느 항목인지 특정할 단서가 없으면 전체 바인딩
        (RELEVANCE 유사) 또는 NEW 로 두되, 과대바인딩(엉뚱한 소수 항목 선택)은 금지.
      예: 직전에 "명화탐정단" 단건을 자세히 안내한 직후 "주말도 참여 가능해?"
          → DRILL, ref_indices=[명화탐정단 인덱스]. NEW→검색으로 보내지 말 것.
  (3) 기상 조건 하 운영여부 후속 → 맥락 후속(REFINE, 초점이 단건이면 DRILL).
      "폭염에도 운영해?", "비 와도 해?", "더워도 하나요?" 처럼 직전에 보여준 시설/초점의
      *운영여부*를 기상 조건과 함께 묻는 후속은, 직전 검색·초점 맥락이 있으면 맥락 후속으로
      분류하세요(NEW/OUT_OF_SCOPE/domain_outside 아님). 예약 시설의 운영여부를 기상과
      함께 묻는 것은 도메인 안(운영성)이고, 순수 날씨 정보 요청("오늘 날씨 어때?")만
      domain_outside 입니다.
      · 직전 초점이 단건이면 그 항목에 바인딩하는 DRILL(ref_indices=[해당 인덱스]).
      · 직전이 목록 맥락이면 그 목록 위에 얹는 REFINE.
"""
...
INTAKE_FEW_SHOT_EXAMPLES = [
    ...
    {
        # 정정/부정 후속 → REFINE(topic 재선언 + 직전 제약 유지).
        "message": (
            "[직전 맥락] 어시스턴트: 성동구 공공서비스 몇 건을 안내드립니다.\n"
            "사용자 메시지: 아니 영화촬영 장소 말이야"
        ),
        "output": _ex(
            "'아니 ... 말이야'는 직전 흐름을 바로잡아 topic(영화촬영 장소)을 다시 "
            "선언하는 정정 후속. 새 지역·카테고리를 도입하는 게 아니라 직전 제약 위에 "
            "올바른 topic 을 재선언하므로 REFINE(NEW 아님).",
            "REFINE",
            "RETRIEVE",
            None,
            [],
            "영화촬영이 가능한 장소로 다시 찾아봅니다.",
        ),
    },
    {
        # 생략형 속성 후속 → DRILL(직전 단일 초점 항목 바인딩).
        "message": (
            "[직전 결과]\n1. 명화탐정단\n"
            "사용자 메시지: 주말도 참여 가능해?"
        ),
        "output": _ex(
            "시설명·서수 없이 속성(주말 참여 가능 여부)만 묻는 생략형 후속. 새 시설/"
            "지역/카테고리 도입 없음 + 직전이 단일 항목(명화탐정단)이므로 그 항목에 "
            "바인딩하는 DRILL. ref_indices=[1].",
            "DRILL",
            "RETRIEVE",
            None,
            [1],
            "명화탐정단의 주말 참여 가능 여부를 확인해드립니다.",
        ),
    },
    {
        # 기상 조건 하 운영여부 후속 → 맥락 후속(직전 단건이므로 DRILL). domain_outside 아님.
        "message": (
            "[직전 결과]\n1. 마루공원 테니스장\n"
            "사용자 메시지: 폭염에도 운영해?"
        ),
        "output": _ex(
            "기상 조건(폭염) 하 운영여부를 묻는 후속. 순수 날씨 질의가 아니라 직전 초점 "
            "시설의 운영여부이므로 도메인 안(운영성). 직전이 단건(마루공원 테니스장)이라 "
            "그 항목에 바인딩하는 DRILL. domain_outside/OUT_OF_SCOPE 아님. ref_indices=[1].",
            "DRILL",
            "RETRIEVE",
            None,
            [1],
            "마루공원 테니스장이 폭염철에도 운영하는지 확인해드릴게요.",
        ),
    },
    {
