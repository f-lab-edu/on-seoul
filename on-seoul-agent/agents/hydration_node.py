...
본 노드는 그 결합을 점진적으로 끊는다. 책임은 다음과 같다.

1. 검색 결과(`vector_results` / `sql_results`)에서 service_id 추출 (`_extract_service_ids`).
2. 통합 슬롯(`state.hydrated_services`)에 원본 행을 모아 후속 단계에 노출.
3. `search_channels.FINAL` 채널 구성 (hydration 적용 후 사용자 노출 목록).
...
뽑으면 충분하다. 같은 정보를 두 슬롯에 중복 보관하면 State 정합성 부담만 늘어난다.

Phase 단계
----------
**Phase 1 (현재)**:
  - `VectorAgent.search()` 가 `hydrate_services` 를 호출하여 `vector_results` 에
    이미 hydrated 행을 채운다 — 기존 28개 테스트 호환을 위해.
  - 본 노드는 vector 경로에서 이중 조회를 피하기 위해 `vector_results` 를 그대로 통과한다.
  - SQL 경로(`sql_results`)도 그대로 통과한다 (sql_search 가 이미 원본 반환).
  - 미래의 새 검색 경로용 `pending_service_ids` 처리는 본 노드에서 제공.

**Phase 2 (별도 PR)**:
  - `VectorAgent.search()` 에서 `hydrate_services` 호출 제거.
  - `vector_results` 를 `[{service_id, rrf_score}]` 메타-only 로 단순화.
  - 본 노드가 `_extract_service_ids` + `hydrate_services` + 메타 머지 + FINAL 채널을 단일 책임으로 처리.
  - 28개 테스트 일괄 수정.
...
class HydrationNode:
    ...
    상태 변화 (Phase 1):
        - intent=VECTOR_SEARCH → vector_results 가 이미 hydrated 이므로 그대로 통과.
          (Phase 2 에서 service_id 추출 + hydrate_services 호출 + 메타 머지로 전환.)
        - intent=SQL_SEARCH    → sql_results 그대로 통과 (이미 원본).
        - 새 검색 경로(pending_service_ids 가 미래에 전달되는 경우)
          → hydrate_services 호출 + 메타 머지 + FINAL 채널.
    ...
    async def __call__(
        ...
        # VECTOR_SEARCH — Phase 1: VectorAgent 가 이미 hydrate 했으므로 그대로 통과.
        # Phase 2 에서 본 노드가 hydrate_services 를 직접 호출하도록 이관 예정.
        if intent == IntentType.VECTOR_SEARCH:
            vector_results = state.get("vector_results") or []
            return {"hydrated_services": list(vector_results)}
