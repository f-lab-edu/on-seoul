...
class _BodyResponse(BaseModel):
    """LLM 구조화 출력 전용 스키마. title은 코드에서 결정적으로 생성한다."""

    body: str


def _make_title(service_count: int) -> str:
    """오늘 날짜와 서비스 건수로 title을 생성한다."""
    today = date.today()
...
async def _invoke_llm(req: NotificationTemplateRequest) -> str:
    """LLM을 호출해 body 텍스트를 반환한다. title은 호출부에서 결정적으로 생성한다.
    ...
    chain = llm.with_structured_output(_BodyResponse)
    result: _BodyResponse = await chain.ainvoke(_build_messages(req))
    return result.body
...
async def create_template(
    ...
    - title: 오늘 날짜 + 서비스 건수로 결정적 생성 (LLM 미사용)
    - body: LLM 생성. self-timeout 8초, 빈 body → 503
    ...
    if not body_text.strip():
        logger.warning(
            "알림 템플릿 LLM이 빈 body 반환. service_ids=%s body=%r",
            service_ids,
            body_text,
        )
        raise HTTPException(status_code=503, detail=_503_DETAIL)
