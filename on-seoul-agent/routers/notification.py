...
그룹 리스트)을 받아, 짧은 요약/하이라이트(title/summary)를 생성한다.

역할: 서비스명·상태·접수기간·링크 같은 "사실"은 소비자(API 서비스)의 Knock
이메일 Liquid 템플릿이 결정적으로 렌더링한다. 이 엔드포인트는 사실 재나열이
아니라 "무엇을 주목할지"만 담은 요약을 생성한다. title은 코드에서 결정적으로
생성하고(날짜·서비스 개수 포맷), summary만 LLM이 생성한다.
...
class _HighlightResponse(BaseModel):
    """LLM 구조화 출력 전용 스키마. summary만 LLM이 생성한다(title은 코드 생성)."""

    summary: str


def _make_title(service_count: int) -> str:
    """알림 title을 코드에서 결정적으로 생성한다(날짜 + 서비스 개수 포맷)."""
    today = date.today()
    return f"온서울 맞춤 {today.month}월 {today.day}일 공공서비스 정보 - {service_count}개"
...
async def _invoke_llm(req: NotificationTemplateRequest) -> _HighlightResponse:
    """LLM을 호출해 summary를 반환한다(title은 코드에서 생성).
    ...
    chain = llm.with_structured_output(_HighlightResponse)
...
async def create_template(
    ...
    - title: 코드에서 결정적으로 생성(날짜 + 서비스 개수). LLM 미사용.
    - summary: LLM 생성. self-timeout 8초.
    - 빈 summary → 503 (title은 코드 생성이라 빈 값 불가).
