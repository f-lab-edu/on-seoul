...
```html
<div style="font-family: -apple-system, 'Apple SD Gothic Neo', sans-serif; max-width: 600px; margin: 0 auto; color: #222;">
  <h2 style="font-size: 18px; margin: 0 0 8px;">{{ data.title }}</h2>
  {% if data.summary %}
  <p style="font-size: 14px; color: #555; margin: 0 0 20px;">{{ data.summary }}</p>
  {% endif %}
  ...
  <p style="font-size: 11px; color: #aaa; margin-top: 24px;">
    본 메일은 ON-Seoul 서비스 변경 구독에 따라 발송되었습니다.
  </p>
</div>
```
