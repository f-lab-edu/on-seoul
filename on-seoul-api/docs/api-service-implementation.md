...
- [ ] `on_ai_app` / `on_data_reader` 권한 부여
...
- [ ] PostgreSQL `app` 스키마 구성 — `users`(소셜 계정 정보), `query_history`
  - `users`: id, provider(`google` 등), provider_id, email, name, created_at — 비밀번호 없음
- [ ] JPA 엔티티 및 Repository
- [ ] Redis 연결 확인 (Refresh Token 저장용 — `spring-boot-starter-data-redis`)

### Phase 13. 인증 엔드포인트

- [ ] Spring Security OAuth2 Client 설정 (`application.yml`에 provider 클라이언트 ID/Secret)
- [ ] OAuth2 로그인 성공 핸들러 — JWT(Access + Refresh) 발급, Refresh Token Redis 저장
- [ ] JWT 검증 필터 — `Authorization: Bearer <token>` 헤더 파싱
- [ ] `POST /auth/token/refresh` — Refresh Token으로 Access Token 재발급
- [ ] `POST /auth/logout` — Refresh Token Redis 삭제 (강제 만료)
- [ ] 인증 예외 처리 (`@ControllerAdvice`) — 401/403 JSON 응답
...
- [ ] JWT 발급 / 검증 / 만료 단위 테스트
- [ ] Refresh Token 갱신 및 무효화 테스트
- [ ] 예외 처리 테스트 (401/403 JSON 응답)
