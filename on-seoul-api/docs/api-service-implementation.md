...
- [x] DB 계정 및 권한 준비 — `on_data_app`, `on_ai_app`, `on_data_reader`
...
- [x] `@Scheduled(cron = "0 0 8 * * *")` — 매일 08시 실행
...
- [x] X/Y null 레코드에 대해 `PLACENM` 기반 카카오 Geocoding API 호출
- [x] Upsert 후처리 또는 별도 배치로 실행
- [x] Geocoding 결과 캐싱 (동일 장소명 반복 호출 방지)

> 카카오 Geocoding 한도 일 30만 건, 본 규모(1000건 미만)에서는 여유

### Phase 9. 수집 파이프라인 단위 테스트 (T7, 병행)

- [x] T2 API 클라이언트 — 응답 Mock(`@WebClientTest` 또는 WireMock)으로 페이지네이션 / 에러 응답 / 타임아웃 검증
- [x] T3 변환기 — 날짜 포맷 예외, 좌표 null, 필드 누락 엣지 케이스
- [x] T4 Upsert — 신규/변경/유지 분류 정확성, `prev_service_status` 갱신
- [x] T5 파이프라인 — 부분 실패 시 나머지 API 진행, 이력 기록 정확성
...
> 실제 구현은 초기 계획과 일부 다르게 진행됐다. `on_ai` DB `public` 스키마 사용, 테이블명 `service_embeddings`로 확정.

- [x] pgvector extension 설치 (`CREATE EXTENSION IF NOT EXISTS vector`)
- [x] `service_embeddings` 테이블 생성 (`on_ai` DB)
  - `id`, `service_id`(=`service_id` UK), `service_name`, `metadata JSONB`, `embedding vector(1536)`
  - 모델: Gemini `gemini-embedding-2-preview` (1536차원)
- [x] `service_id` 인덱스
- [x] `on_ai_app` / `on_data_reader` 권한 부여
...
- [x] Gradle Wrapper 구성 — `build.gradle.kts`, `settings.gradle.kts`
- [x] 의존성 정의 — Spring Web, Security, Data JPA, Redis, PostgreSQL Driver
- [x] 패키지 레이아웃 — `controller/`, `service/`, `domain/`, `repository/`, `scheduler/`, `security/`
- [x] `application.yml` — 프로파일별 DB / Redis 접속 정보
- [x] 헬스체크 엔드포인트
...
## Epic 5 — `feat/auth`
> Phase 12–13 | OAuth 2.0 소셜 로그인 + JWT 인증 동작 확인

인증은 핵심 도메인이 아니므로 구현 비용을 최소화한다.
소셜 로그인으로 인증을 외부에 위임하고, 내부 서비스 간 통신은 JWT로 처리한다.

- **Access Token** — 만료 짧게 (15~30분), 서버 상태 저장 없음
- **Refresh Token** — 만료 길게 (7일), Redis에 저장하여 강제 만료 가능 (토큰 탈취 대응)

### Phase 12. 스키마 및 도메인

- [x] PostgreSQL `app` 스키마 구성 — `users`(소셜 계정 정보), `query_history`
  - `users`: id, provider(`google` 등), provider_id, email, name, created_at — 비밀번호 없음
- [x] JPA 엔티티 및 Repository
- [x] Redis 연결 확인 (Refresh Token 저장용 — `spring-boot-starter-data-redis`)

### Phase 13. 인증 엔드포인트

- [x] Spring Security OAuth2 Client 설정 (`application.yml`에 provider 클라이언트 ID/Secret)
- [x] OAuth2 로그인 성공 핸들러 — JWT(Access + Refresh) 발급, Refresh Token Redis 저장
- [x] JWT 검증 필터 — `Authorization: Bearer <token>` 헤더 파싱
- [x] `POST /auth/token/refresh` — Refresh Token으로 Access Token 재발급
- [x] `POST /auth/logout` — Refresh Token Redis 삭제 (강제 만료)
- [x] 인증 예외 처리 (`@ControllerAdvice`) — 401/403 JSON 응답

검증: 소셜 로그인 → Access Token 수령 → 보호 리소스 접근 → 토큰 갱신 → 로그아웃 플로우 확인
...
- [x] `POST /query` — 인증된 사용자 질의 수신
- [x] AI 서비스 `/chat/stream` 호출 및 SSE 릴레이
- [x] `query_history` 이력 저장 (ChatRoom + ChatMessage — USER/ASSISTANT)
- [x] 에러 / 타임아웃 / 재시도 정책

검증: Access Token으로 `/query` 호출 → 자연어 응답 수신 + `query_history` 레코드 확인
...
- [x] JWT 발급 / 검증 / 만료 단위 테스트
- [x] Refresh Token 갱신 및 무효화 테스트
- [x] 예외 처리 테스트 (401/403 JSON 응답)
- [x] `/query` 통합 테스트 (AI 서비스 모킹)
- [x] `./gradlew test` 전 구간 통과
...
- Redis는 Refresh Token 저장 전용 (`RT:{userId}` 키, TTL = Refresh Token 만료 시간)
