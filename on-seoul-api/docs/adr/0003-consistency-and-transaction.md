...
| ChangeLog 작성 + Embeddings 재생성 | **최종 일관성 (≤5분)** | PG 1 TX (ChangeLog) + AI 서비스 REST API 호출 (별도) | ChangeLog 작성은 강한 일관성. Embeddings는 수집 TX 커밋 직후 별도 워커가 AI 서비스 REST API를 호출. 임베딩 벡터 생성·pgvector 적재는 AI 서비스 책임. 실패 시 다음 수집 배치에서 재호출 |
...
- **갱신 트리거:** 수집 트랜잭션 커밋 직후 `EmbeddingSyncQueue`에 `service_id`를 enqueue. 별도 워커(Spring `@Async` 또는 `ScheduledExecutor` 기반)가 큐를 소비해 **AI 서비스 REST API를 호출**한다. 임베딩 벡터 생성과 `service_embeddings` 테이블(pgvector) 적재는 AI 서비스(`on-seoul-agent`) 책임. 메시지 브로커 없음.
