...
- **갱신 트리거:** 수집 트랜잭션 커밋 직후 `EmbeddingSyncQueue`에 `service_id`를 enqueue. 별도 워커(Spring `@Async` 또는 `ScheduledExecutor` 기반)가 큐를 소비해 AI 서비스에서 임베딩 벡터를 생성한 후 `service_embeddings` 테이블(pgvector)에 upsert.
