...
저장소는 세 종류다: 단일 PostgreSQL(여러 BC의 정형 데이터), Qdrant(임베딩), 외부 푸시 시스템. PG 내부는 단일 TX로 강한 일관성이 가능하지만, Qdrant와 외부 호출은 TX 외부이므로 최종 일관성이 필수다.
...
PG 내부 작업은 **강한 일관성, 1 TX**. PG↔Qdrant 및 외부 호출은 **최종 일관성**으로 처리하되, **부정합 허용 윈도우 = 5분 SLA**를 명시하고 metrics로 감시한다.
...
| ChangeLog 작성 + Embeddings 재생성 | **최종 일관성 (≤5분)** | PG 1 TX + Qdrant 별도 | ChangeLog 작성은 강한 일관성. Embeddings는 수집 TX 커밋 직후 별도 워커가 비동기 호출. 실패 시 `content_hash` 비교 기반 다음 수집 배치에서 자동 복구 |
...
- **목표 시간차:** ChangeLog 커밋 후 5분 이내 Qdrant 반영.
- **갱신 트리거:** 수집 트랜잭션 커밋 직후 `EmbeddingSyncQueue`에 `service_id`를 enqueue. 별도 워커(Spring `@Async` 또는 `ScheduledExecutor` 기반)가 큐를 소비해 Qdrant upsert.
- **부정합 시 사용자 경험:** 알림을 받은 시점 ~ 임베딩 갱신 완료 전 윈도우(최대 5분)에 채팅으로 같은 서비스를 질의하면, Vector 라우팅 결과가 구버전을 반환할 수 있다. 이 경우 SQL 라우팅은 영향 없음(PG가 최신). MVP 허용 범위.
...
- Embeddings 워커가 장기 실패하면 `embeddings.sync.lag.seconds`가 누적 — 알림 기능과는 독립이지만 채팅 품질에 영향. metric alert 필수.
