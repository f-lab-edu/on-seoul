...
> **현재 구현**(`KnockNotificationAdapter`)은 `recipients`에 userId 문자열만 전달한다. 실제 이메일/전화번호를 함께 전달하려면 `NotificationScheduler` → `KnockNotificationAdapter`에 사용자 연락처 조회 로직을 추가해야 한다.
