...
class PublicServiceReservationPersistenceAdapterTest {
    ...
    void findAllByCoordXIsNullOrCoordYIsNull_returnsNullCoordRecordsOnly() {
        adapter.save(buildReservation("SVC-010", null, null, null));
        adapter.save(buildReservation("SVC-011", new BigDecimal("126.9"), null, null));
        adapter.save(buildReservation("SVC-012",
                new BigDecimal("126.9"), new BigDecimal("37.5"), null));
    ...
    void findAllByDeletedAtIsNull_excludesSoftDeleted() {
        adapter.save(buildReservation("SVC-020", null, null, null));
        adapter.save(buildReservation("SVC-021", null, null, LocalDateTime.now()));
